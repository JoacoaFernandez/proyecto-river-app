...
let PushService = PushService_1 = class PushService {
    ...
    async saveSubscription(endpoint, keys, userId) {
    ...
    async sendToAll(payload) {
        ...
        const results = await Promise.allSettled(subs.map((sub) => webpush.sendNotification({ endpoint: sub.endpoint, keys: sub.keys }, JSON.stringify(payload))));
