                  ...
                  return (
                    <li
                      ...
                      className={`flex items-center gap-2 py-1 border-b border-neutral-800/50 last:border-0 ${
                        replacementIds.has(p.id) ? 'bg-emerald-950/20 rounded px-1' : isInactive ? 'opacity-60' : ''
                      }`}
                    >
                      ...
                      <span className={`flex-1 truncate ${isInactive ? 'text-neutral-500 line-through decoration-neutral-700' : 'text-neutral-300'}`}>{p.name}</span>
